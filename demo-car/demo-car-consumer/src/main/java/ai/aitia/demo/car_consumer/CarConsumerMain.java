...
public class CarConsumerMain implements ApplicationRunner {
	...
	public void run(final ApplicationArguments args) throws Exception {
    	...
    	//updateCarServiceOrchestrationAndConsumption();
    	//getCarServiceOrchestrationAndConsumption();
    ...
    //-------------------------------------------------------------------------------------------------
    /*public void updateCarServiceOrchestrationAndConsumption() {
    	logger.info("Orchestration request for " + CarConsumerConstants.UPDATE_CAR_SERVICE_DEFINITION + " service:");
    	final ServiceQueryFormDTO serviceQueryForm = new ServiceQueryFormDTO.Builder(CarConsumerConstants.UPDATE_CAR_SERVICE_DEFINITION)
    																		.interfaces(getInterface())
    																		.build();
		...
		if (orchestrationResponse == null) {
			...
			final OrchestrationResultDTO orchestrationResult = orchestrationResponse.getResponse().get(0);
			validateOrchestrationResult(orchestrationResult, CarConsumerConstants.UPDATE_CAR_SERVICE_DEFINITION);
			
			final List<CarRequestDTO> carsToUpdate = List.of(new CarRequestDTO("nissan", "red"), new CarRequestDTO("mazda", "red"));
			int carId = 1;
			for (final CarRequestDTO carRequestDTO : carsToUpdate) {
				logger.info("Update a car request:");
				printOut(carRequestDTO);
				final String token = orchestrationResult.getAuthorizationTokens() == null ? null : orchestrationResult.getAuthorizationTokens().get(getInterface());
				final CarResponseDTO carUpdated = arrowheadService.consumeServiceHTTP(CarResponseDTO.class, HttpMethod.valueOf(orchestrationResult.getMetadata().get(CarConsumerConstants.HTTP_METHOD)),
						orchestrationResult.getProvider().getAddress(), orchestrationResult.getProvider().getPort(), orchestrationResult.getServiceUri() + "/" + carId,
						getInterface(), token, carRequestDTO, new String[0]);
				carId++;
				logger.info("Provider response");
				printOut(carUpdated);
